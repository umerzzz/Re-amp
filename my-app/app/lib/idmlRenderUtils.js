...
// Font handling utilities
let loadedFonts = new Set(); // Track loaded fonts to avoid duplicates
  ...
  try {
    ...
    for (const font of fonts) {
      ...
      const fontPath = `/api/serve-font?uploadId=${encodeURIComponent(
        uploadId
      )}&fontName=${encodeURIComponent(font.name)}`;
  ...
  if (story) {
    ...
    for (const p of paras) {
            ...
            Object.entries(paraStyle.Properties).forEach(([key, value]) => {
              if (value && value["#text"] !== undefined) {
                ...
                if (key === "AppliedFont") {
                  ...
                  pEl.style.fontFamily = `"${cleanedBaseName}", "${cleanedFullName}", sans-serif`;
                  // Store the original font name as a data attribute for debugging
                  pEl.dataset.originalFont = propValue;
                  pEl.dataset.fontFamily = baseFontName;
                  pEl.dataset.cleanedFontFamily = cleanedBaseName;
                }
      ...
      for (const s of p.spans || []) {
              ...
              Object.entries(charStyle.Properties).forEach(([key, value]) => {
                if (value && value["#text"] !== undefined) {
                  ...
                  if (key === "AppliedFont") {
                    ...
                    const cleanedFullName = cleanFontName(propValue);
                    // Set font family with fallbacks
                    span.style.fontFamily = `"${cleanedBaseName}", "${cleanedFullName}", sans-serif`;
                    // Store the original font name as a data attribute for debugging
                    span.dataset.originalFont = propValue;
                    span.dataset.fontFamily = baseFontName;
                    span.dataset.cleanedFontFamily = cleanedBaseName;
                  }
        ...
        if (s.font) {
          // Extract the base font family name
          const baseFontName = extractFontFamily(s.font);
          // Clean font names for CSS
          const cleanedBaseName = cleanFontName(baseFontName);
          const cleanedFullName = cleanFontName(s.font);
          // Set font family with fallbacks
          span.style.fontFamily = `"${cleanedBaseName}", "${cleanedFullName}", sans-serif`;
          // Store the original font name as a data attribute for debugging
          span.dataset.originalFont = s.font;
          span.dataset.fontFamily = baseFontName;
          span.dataset.cleanedFontFamily = cleanedBaseName;
        }
